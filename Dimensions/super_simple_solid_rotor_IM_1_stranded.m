...
dim.p = 1; %pole-pairs
dim.symmetry_sectors = dim.p; %symmetry sectors in FEA model
...
% setting conductor layout
layout = RoundWireLayout();
layout.diameter = 0.8e-3;
%layout.bundling_algorithm = 'none'; %just default ordering
layout.bundling_arguments.y_weight = 1;

%winding transpositions between blocks
trsp_args = struct();
trsp_args.type = 'block2024';
trsp_args.number_of_blocks = 3;
winding_spec.strand_transposition_type = trsp_args;
%winding_spec.strand_transposition_type = 'none';

winding_spec.layout_spec = layout;
winding_spec.wires_in_hand = 33;
winding_spec.winding_model_type = defs.custom;
...
stator.visualize();
visualize_strand_layout(layout, 'plot_numbers', false);